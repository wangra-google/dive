...
static constexpr char *kViewModeStrings[kViewModeStringCount] = { "Submit", "Events" };
static constexpr char *kEventViewModeStrings[kEventViewModeStringCount] = { "GPU Events" };
static constexpr char *kFilterStrings[DiveFilterModel::kFilterModeCount] = {
    "None",
    "BinningPassOnly",
    "FirstTilePassOnly",
...
MainWindow::MainWindow()
{
    ...
    constexpr DiveFilterModel::FilterMode kDefaultFilterMode = DiveFilterModel::kNone;
    {
        ...
        QFrame *text_combo_box_frame = new QFrame();
        {
            ...
            for (int i = 0; i < DiveFilterModel::kFilterModeCount; i++)
            {
...
void MainWindow::OnSelectionChanged(const QModelIndex &index)
{
    ...
    const Dive::CommandHierarchy &command_hierarchy = m_data_core->GetCommandHierarchy();
    uint64_t       selected_item_node_index = (uint64_t)(source_index.internalPointer());
...
void MainWindow::OnFilterModeChange(const QString &filter_mode)
{
    ...
    if (m_filter_model)  // Always good to null-check pointers
    {
        m_filter_model->SetMode(new_mode);
    }

    // Scroll to the top to see the new beginning of the filtered list.
